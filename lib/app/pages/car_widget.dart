...
/// Separate widget for the fixed image card
class CarWidgetImageCard extends StatefulWidget {
  const CarWidgetImageCard({required this.state, super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.only(
        top: MediaQuery.of(context).padding.top + 26.h, // SafeArea + 8 extra
      ),
      child: _HeroImageCard(state: widget.state, vehicle: vehicle, isDark: isDark),
    );
  }
}
...
/// Separate widget for the scrollable specs container
class CarWidgetSpecsContainer extends StatefulWidget {
  const CarWidgetSpecsContainer({required this.state, super.key});
...
class _CarWidgetSpecsContainerState extends State<CarWidgetSpecsContainer> {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: _CarWidgetSpecsOnly(state: widget.state),
    );
  }
}

/// Internal widget that only shows specs and awards
class _CarWidgetSpecsOnly extends StatelessWidget {
  const _CarWidgetSpecsOnly({required this.state});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 12.h),
            _buildAwardsSectionStatic(context),
            SizedBox(height: 20.h),
            // Specs grid
            Text(
              'Specifications',
            ...
            SizedBox(height: 12.h),
            _buildSpecsGridStatic(state, isDark),
          ],
        ),
      ),
    );
  ...
  static Widget _buildSpecsGridStatic(ProfileState state, bool isDark) {
    final vehicle = state.vehicles.isNotEmpty ? state.vehicles.first : null;
